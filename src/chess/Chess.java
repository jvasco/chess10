...
public class Chess {
	...
	public static void main(String[] args) {
		...
		while (game) {
			...
			while (piece == null || !piece.isValidMove(chessBoard, move) || (turn == -1 && !piece.isWhite()) || (turn == 1 && piece.isWhite()) ) {
			...
			if (chessBoard[f1][r1].isValidMove(chessBoard, move)) {
				...
				if(!enp){
					if(chessBoard[f1][r1] != null && chessBoard[f1][r1].getName().equals("wp") && f1 == 6 && f2 == 4){
						System.out.println("PIsdfsdfece is: " + chessBoard[f1][r1].getName() + " f1 f2" + f1 + f2);
						enp = true;
						fe = f2;
						re = r2;
					}else if(chessBoard[f1][r1] != null && chessBoard[f1][r1].getName().equals("bp") && f1 == 1 && f2 == 3){
						enp = true;
						fe = f2;
						re = r2;
					}
		...
		for (int i = 0; i < board.length; i++) {
			for (int j = 0; j < board[i].length; j++) {
				...
				if (turn == -1 && !board[i][j].isWhite()) {
					...
					move = pos + " " + tempWK;
					if (board[i][j].isValidMove(board, move)) {
						System.out.println("Piece is: " + board[i][j].getName() + " and move is: " + move);
						return true;
					}
				} else if (turn == 1 && board[i][j].isWhite()) {
					// blacks turn
